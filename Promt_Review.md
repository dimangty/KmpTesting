...
Ничего не пиши до строки `1. Находки`.
Ответ, который начинается с пересказа вроде `This is`, `В этом diff`, `Код добавляет`, `Реализован экран`, является невалидным и должен быть переписан.
Это не задача объяснения кода. Пересказ реализации запрещён.
...
- Перед отправкой проверь первую строку ответа: она должна быть ровно `1. Находки`.
- Если после чтения diff ты начал писать summary, остановись и полностью перепиши ответ по шаблону.
